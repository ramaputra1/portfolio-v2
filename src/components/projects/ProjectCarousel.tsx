...
const STATUS_CHIPS: { value: StatusFilter; label: string; dot: string }[] = [
  ...
  { value: "private", label: "Private", dot: "bg-yellow-400" },
];

function matchStatus(p: { liveUrl: string; githubUrl: string }, filter: StatusFilter) {
...
export function ProjectCarousel({ projects }: { projects: Project[] }) {
  ...
  const current = filtered.slice(safePage * PER_PAGE, (safePage + 1) * PER_PAGE);
    ...
    <>
      ...
      <div className="mt-8 flex flex-col gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={() => { setSortLatest((v) => !v); resetPage(); }}
          ...
          {STATUS_CHIPS.map(({ value, label, dot }) => {
            ...
            return (
              <button
                key={value}
                onClick={() => { setStatus(active ? null : value); resetPage(); }}
                className={cn(
              ...
              >
                <span className={cn("inline-block h-2 w-2 rounded-full", active ? dot : "bg-text-muted")} />
                {label}
              </button>
            );
          })}
        ...
        <div className="flex items-center gap-3">
          <ArrowBtn onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={!canPrev}>
            <ChevronLeft size={20} />
          </ArrowBtn>
          ...
          <ArrowBtn onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))} disabled={!canNext}>
            ...
            {Array.from({ length: totalPages }).map((_, i) => (
              <button
                ...
                className={cn(
                  ...
                  i === safePage ? "w-6 bg-primary" : "w-2 bg-border hover:bg-border-light",
                )}
              />
            ))}
          </div>
